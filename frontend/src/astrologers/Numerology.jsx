...
export default function Numerology() {
  ...
  useEffect(() => {
    const fetchNumerologyAdvisors = async () => {
      try {
        ...
        if (res.data.success) {
          ...
          console.log("Fetched numerology psychics:", psychicsData);
          ...
          // Calculate tab counts based on API data
          setTabCounts({
        ...
        console.error("Error fetching numerology advisors:", err);
        setPsychics([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <>
      <CategoryHeader
        title="Numerology"
        description="Our Numerology specialists can see and interpret the colors and energies surrounding you, providing insights into your emotional, mental, and spiritual state."
                ...
                <TabsContent value="active">
                  <div className="grid gap-6">
                    {loading ? (
                      ...
                      <p className="text-center mt-6">No numerology advisors available.</p>
                    ) : (
      ...
      <Dialog open={showAstroForm} onOpenChange={setShowAstroForm}>
        <DialogContent className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl max-h-[80vh] overflow-y-auto">
          <div className="p-4">
            <h2 className="text-xl font-semibold mb-4 text-center">
              Numerology Reading with {selectedPsychic?.name}
            </h2>
            <form
              onSubmit={async (e) => {
                e.preventDefault();
